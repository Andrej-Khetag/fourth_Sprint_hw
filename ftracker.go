...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkingMeanSpeed := meanSpeed(action, duration)
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(walkingMeanSpeed, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
